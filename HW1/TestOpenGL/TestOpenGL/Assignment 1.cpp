...
//Display options for this program.
	chooseOption();
...
/*---------------------------------------------------------------------
chooseOption(): Display options for this program. There are three mod-
es for this program. Option 1 is to draw one single circle, the x, y 
and radius are given by user. Option 2 is to draw several circle fro-
m the given text file. Option 3 is to draw the aniamtion of these ci-
rcles in option 2.
---------------------------------------------------------------------*/
void chooseOption(void)
{
...
void file_in(void)
{
	...
	//adjust the x,y and r value in order to fit into the Window
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < 3; j++)
		{
...
void idle(void)
{
	//When no others functions be called, idle function be called and draw animation of the circles.
	frame++;
	if (frame > K) frame = 1;
	display();
